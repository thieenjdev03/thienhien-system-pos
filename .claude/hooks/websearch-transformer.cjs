#!/usr/bin/env node
/**
 * WebSearch Hook - CLI Tool Executor with Fallback Chain
 *
 ...
 *   WEBSEARCH_SKIP=1           - Skip this hook entirely (for official Claude)
 *   WEBSEARCH_ENABLED=1        - Enable WebSearch (default: 1)
 *   WEBSEARCH_TIMEOUT=55       - Timeout in seconds (default: 55)
 *   WEBSEARCH_GEMINI=1         - Enable Gemini CLI provider
 *   WEBSEARCH_GEMINI_MODEL     - Gemini model (default: gemini-2.5-flash)
 *   WEBSEARCH_OPENCODE=1       - Enable OpenCode provider
 *   WEBSEARCH_GROK=1           - Enable Grok CLI provider
 *   WEBSEARCH_OPENCODE_MODEL   - OpenCode model (default: opencode/grok-code)
 *   CK_DEBUG=1                    - Enable debug output
 *
 * Exit codes:
...
// Load .claude/.env if exists (for WEBSEARCH_* settings)
const claudeEnvPath = path.join(__dirname, '..', '.env');
...
const PROVIDER_CONFIG = {
  ...
  opencode: {
    // Model to use (can be overridden via WEBSEARCH_OPENCODE_MODEL env var)
...
function shouldSkipHook() {
  if (process.env.WEBSEARCH_SKIP === '1') return true;

  // Explicit disable
  if (process.env.WEBSEARCH_ENABLED === '0') return true;
...
function isProviderEnabled(provider) {
  const envVar = `WEBSEARCH_${provider.toUpperCase()}`;
...
async function processHook() {
  try {
    ...
    const timeout = parseInt(process.env.WEBSEARCH_TIMEOUT || DEFAULT_TIMEOUT_SEC, 10);
    ...
    const enabledProviders = providers.filter((p) => {
      ...
      if (process.env.CK_DEBUG) {
        console.error(`[WebSearch] ${p.name}: enabled=${enabled}, available=${available}`);
      }
    ...
    if (process.env.CK_DEBUG) {
      ...
      console.error(`[WebSearch] Enabled providers: ${names}`);
    }

    // Try each enabled provider in order
    for (const provider of enabledProviders) {
      if (process.env.CK_DEBUG) {
        console.error(`[WebSearch] Trying ${provider.name}...`);
      }
      ...
      if (process.env.CK_DEBUG) {
        console.error(`[WebSearch] ${provider.name} failed: ${result.error}`);
      }
    ...
    if (process.env.CK_DEBUG) {
      console.error('[WebSearch] Parse error:', err.message);
    }
    process.exit(0);
  }
}
...
function tryGeminiSearch(query, timeoutSec = DEFAULT_TIMEOUT_SEC) {
  try {
    ...
    const model = process.env.WEBSEARCH_GEMINI_MODEL || config.model;

    if (process.env.CK_DEBUG) {
      console.error(`[WebSearch] Executing: gemini --model ${model} --yolo -p "..."`);
    }
...
function tryOpenCodeSearch(query, timeoutSec = DEFAULT_TIMEOUT_SEC) {
  try {
    ...
    // Allow model override via env var
    const model = process.env.WEBSEARCH_OPENCODE_MODEL || config.model;
    const prompt = buildPrompt('opencode', query);

    if (process.env.CK_DEBUG) {
      console.error(`[WebSearch] Executing: opencode run --model ${model} "..."`);
    }
...
function tryGrokSearch(query, timeoutSec = DEFAULT_TIMEOUT_SEC) {
  try {
    ...
    if (process.env.CK_DEBUG) {
      console.error('[WebSearch] Executing: grok "..."');
    }
...
function outputNoProvidersEnabled(query) {
  const message = [
    ...
    '',
    'Run install one of the following CLI tools:',
    '',
