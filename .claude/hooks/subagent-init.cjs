...
async function main() {
  try {
    ...
    lines.push(`ID: ${agentId} | Repository: ${path.basename(payload.cwd || process.cwd())}`);
    lines.push(``);
    ...
    // Core rules (minimal)
    lines.push(`## Rules`);
    lines.push(`- **File paths in reports**: Use relative paths from project root (e.g., \`libs/proto-schema/package.json\` NOT absolute paths)`);
    lines.push(`- Reports → ${reportsPath}`);
