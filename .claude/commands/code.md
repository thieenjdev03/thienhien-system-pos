...
argument-hint: [plan] [--model claude|gemini|auto]
---

## Arguments Parser
Parse $ARGUMENTS to extract:
- `$PLAN`: Plan file path or name (default: auto-detect latest)
- `$MODEL`: Execution model (default: `auto`)
  - Extract from flags: `--model=<value>` or `--model <value>`
  - Valid values: `claude`, `gemini`, `auto`
  - If not specified, use `auto` (auto-select based on task type)

**Example inputs:**
- `/code` → plan=auto, model=auto
- `/code plan-auth` → plan=plan-auth, model=auto
- `/code --model gemini` → plan=auto, model=gemini
- `/code plan-auth --model claude` → plan=plan-auth, model=claude
...
**If $MODEL is `auto`:**
- Analyze plan content to determine complexity
- **Use Claude**: For implementation tasks with precise file editing, complex refactoring, strict type systems
- **Use Gemini**: For large codebase analysis (2M context), research-heavy tasks, visual/multimodal needs
- Default: `claude` for most implementation tasks

**If $MODEL is `claude`:**
- Execute with Claude Code subagents (current behavior)
- Precise editing tools, structured workflow, subagent orchestration

**If $MODEL is `gemini`:**
- Delegate to Gemini CLI via `.claude/scripts/ai-delegate.sh`
- Large context window, fast analysis, good for exploration

**Output model selection:** `🤖 Model: [claude|gemini] (Reason: [auto-selected|user-specified])`
...
Execute using Gemini CLI delegation:

```bash
./.claude/scripts/ai-delegate.sh implement \
  "Implement plan: $PLAN following workflow steps 0-6" \
  --model gemini \
  --context "$PLAN" \
  --output "plans/reports/gemini-implementation-$(date +%Y%m%d-%H%M%S).md"
```

After Gemini completes:
1. Review output report
2. Verify implementation against plan
3. Ask user if they want to continue with testing/review steps using Claude subagents
4. **STOP** - Do not proceed with Claude workflow below

---

**IF $MODEL is `claude` (or auto-selected as claude):**

Proceed with Claude Code workflow below (Steps 0-6).
...
Implement selected plan phase step-by-step following extracted tasks (Step 2.1, Step 2.2, etc.). Mark tasks complete as done. For UI work, activate `shadcn-ui-designing` skill: "Implement [feature] UI following shadcn/ui principles and ./docs/design-guidelines.md". Use `ai-multimodal` skill for image assets, `imagemagick` for editing. Run type checking and compile to verify no syntax errors.
