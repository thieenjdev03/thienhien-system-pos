...
argument-hint: [plan] [all-phases-yes-or-no] [--model claude|gemini|auto]
---

## Arguments Parser
Parse $ARGUMENTS to extract:
- `$PLAN`: Plan file path or name (default: auto-detect latest)
- `$ALL_PHASES`: Complete all phases in one run? (default: `yes`)
  - `yes`: Finish all phases automatically
  - `no`: Implement phase-by-phase and wait for confirmation
- `$MODEL`: Execution model (default: `auto`)
  - Extract from flags: `--model=<value>` or `--model <value>`
  - Valid values: `claude`, `gemini`, `auto`

**Example inputs:**
- `/code:auto` → plan=auto, all_phases=yes, model=auto
- `/code:auto plan-auth no` → plan=plan-auth, all_phases=no, model=auto
- `/code:auto --model gemini` → plan=auto, all_phases=yes, model=gemini
- `/code:auto plan-auth yes --model claude` → plan=plan-auth, all_phases=yes, model=claude
...
**If $MODEL is `auto`:**
- Default: `claude` for AUTO mode (trusted automation with subagent coordination)
- Can override to `gemini` for fast, large-context processing
...
Execute using Gemini CLI delegation in auto mode:

```bash
./.claude/scripts/ai-delegate.sh implement \
  "AUTO-IMPLEMENT plan: $PLAN. Complete all phases: $ALL_PHASES. Follow workflow steps 0-5 automatically." \
  --model gemini \
  --context "$PLAN" \
  --output "plans/reports/gemini-auto-implementation-$(date +%Y%m%d-%H%M%S).md"
```

Note: Gemini will execute in `--yolo` mode for implementation (supervised by default).

After completion: Review report and ask user for next steps.

**STOP** - Do not proceed with Claude workflow below.

---

**IF $MODEL is `claude` (or auto-selected as claude):**

Proceed with Claude Code AUTO workflow below (Steps 0-5).
...
Implement selected plan phase step-by-step following extracted tasks (Step 2.1, Step 2.2, etc.). Mark tasks complete as done. For UI work, activate `shadcn-ui-designing` skill: "Implement [feature] UI following shadcn/ui principles and ./docs/design-guidelines.md". Use `ai-multimodal` skill for image assets, imagemagick in `media-processing` skill for editing. Run type checking and compile to verify no syntax errors.
